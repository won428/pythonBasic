...
print('변수 emails는 메일의 제목과 스팸 여부를 tuple 형식으로 가지고 있는 list입니다.')
...
print('변수 subject는 이메일 제목, label은 스팸 여부 문자열입니다.')
...
print(f'독립 변수(메일제목) {num_to_show}개 보기')
...
print(f'문서(이메일 제목)의 갯수는 {x.shape[0]}이고, 출현한 단의 갯수는 {x.shape[1]}개 입니다.')

vocab = vectorizer.vocabulary_
inv_vocab= {v : k for k, v in vocab.items()} # dict comprehension
...
# Multinomia1NB : 범주형 데이터에 사용하는 나이브 베이즈 클래스
# 베이즈 정리 : 조건부 확률에 기반한 ...
print('\n모델 생성')
model = MultinomialNB()
...
final_email_info = [] # 최종 결과를 저장할 리스ㅡ
